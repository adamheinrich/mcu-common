...
#define TEST_RUN(name)		test_run(#name, &(name))

#define TEST_PRINTF(...)	do { \
		uart_printf("[%s():%d] ", __func__, __LINE__); \
		uart_printf(__VA_ARGS__); \
	} while(0)

#define TEST_ASSERT(val)	do { \
		if (!(val)) { \
			TEST_PRINTF("Assertion failed\n"); \
			return false; \
		} \
	} while (0)

bool test_run(const char *name, bool (*test)(void));
